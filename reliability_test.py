...
def server_calibration_2bts(fchs, auth_pass_thres, nrof_batches, nrow, fcsaRes, swapufRes1, swapufRes2):
    ...
    return auth_calibrat_result, cal_2bt_success
...
print("reliability of CPPUF from reliable challenges(stbmask): {}".format((csaRes==gt_csaRes).sum()/nrof_batched_chs))
...
print("reliability of CPPUF from random challenges: {}".format((rcsaRes==rgt_csaRes).sum()/nrof_batched_chs))
...
if succauth_rate != 1:
    ...
    calibrated, cal_2bts = server_calibration_2bts(fchs, auth_pass_thres, len(fchs), nrow, fcsaRes, fswapuf1, fswapuf2)
    print("successful authentication rate (reliable, with 2-bit calibration): {}".format((succmsk.sum()+calibrated.sum())/nrof_batches))

print("CPPUF 1-bit cal Count = {}".format(calibrated.sum()-cal_2bts.sum()))
print("successful authentication rate (reliable, with 1-bit calibration)".format(calibrated.sum()-cal_2bts.sum()))
print("CSAPUF 2-bit cal Count = {}".format(cal_2bts.sum()))

pfr = utils.false_rejected(dim, ap1rel, ap2rel, auth_pass_thres)
print("Approx. CPPUF P_FR (w 1-bit cal) = {}".format(pfr))
#===========================================================
...
rsuccmsk = rauthrate >= auth_pass_thres
rsuccauth_rate = rsuccmsk.sum()/nrof_batches
